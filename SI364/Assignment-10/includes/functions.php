...
function login($email, $password, $pdo) {
    ...
    if ($stmt = $pdo->prepare("SELECT id, username, password, salt 
        ...
        $stmt->execute(array(":email" => $email));// Bind "$email" to parameter.
        $row = $stmt->fetch(PDO::FETCH_ASSOC);// Execute the prepared query.
        $user_id = $row['id'];
        ...
        if ($stmt->rowCount() == 1) {
            ...
            if (checkbrute($user_id, $pdo) == true) {
                ...
                if ($db_password == $password) {
                    ...
                    if (!$stmt->execute("INSERT INTO login_attempts(user_id, time) 
                                            VALUES ('$user_id', '$now')")) {
...
function checkbrute($user_id, $pdo) {
    ...
    if ($stmt = $pdo->prepare("SELECT time 
        ...
        $stmt->execute(array(":user_id" => $user_id));// Bind "$email" to parameter.
        $row = $stmt->fetch(PDO::FETCH_ASSOC);// Execute the prepared query.
...
function login_check($pdo) {
    ...
    if (isset($_SESSION['user_id'], $_SESSION['username'], $_SESSION['login_string'])) {
        ...
        if ($stmt = $pdo->prepare(" SELECT password 
				                    FROM members 
				                    WHERE id = :id LIMIT 1")) {
            $stmt->execute(array(":id" => $user_id));// Bind "$email" to parameter.
            $row = $stmt->fetch(PDO::FETCH_ASSOC);// Execute the prepared query.
