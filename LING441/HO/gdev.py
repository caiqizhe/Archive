    ...
    def get_sem (self, i=None):
        ...
        for (t, tree) in enumerate(trees):
            ...
            if e:
                if self.__trans:
                    te = self.__trans[i]
                    if te != e: bad_trans = True
                if self.__model is not None:
                    v = self.__model.satisfy(e, self.__assignment)
            yield (t, tree, te, e, bad_trans, v)

    def sem (self, i=None):
        try:
            for (t, tree, te, e, bad_trans, v) in self.get_sem(i):
                print('[%d]' % t)
                print(tree)
                if te is not None:
                    print('Target: ', te)
        ...
        for i in range(len(self.__sents)):
            try:
                ...
                if len(sems) == 0:
                    ...
                    for (t, tree, te, e, bad_trans, v) in sems:
                        if bad_trans: star = '*'
                        else: star = ' '
                        if e is None: e = '(no translation)'
                        if v is None: v = '(no value)'
                        print('[%d]' % i, t, star, e, v)
            except Exception as e:
                print('[%d]' % i, 'ERROR:', e)
