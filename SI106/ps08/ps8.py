...
car_method_description = "display all the specification of the car for the user, like color, brand, year"
...
def possible(word, blanked, guesses_made):
    # print( word, blanked, guesses_made)
    for g in guesses_made:
        if g in word:
            if word.count(g) != blanked.count(g):
                return False
    return True
...
def possible_words(blanked, guesses_made, total_words):
    ...
    for w in total_words:
        if possible(w, blanked, guesses_made):
            result.append(w)
    return result
...
for v in [1, 2, 3]: # there're three version iterate through each version
    print "----version", v  # print out version number
    total_guesses = 0   # keeps a local variable for total guesses have made so far
    total_min_guesses = 0   # keeps a local variable for total minimum guesses
    
    for i in range(30): # create 30 games 
        print i, ".", # print out the current game number
        
        (guesses, min_guesses) = play_game(version = v, manual=False) # play_game regurns a tuple and store that to guesses and mim_guesses respectively
        
        total_guesses = total_guesses + guesses # update total_guesses with guesses have made so far
        total_min_guesses = total_min_guesses + min_guesses # update total_min_guesses with current min guesses required for that game
    print # print an an empty line
    print "ratio:", total_guesses / float(total_min_guesses), ";total guesses:", total_guesses, ";min needed:", total_min_guesses 
    # calculate the ratio => version 3 has the lowest ratio as its the optimal strategy and version 1 has the highest ratio as it's just the naive way.
...
class Pet():
    ...
    def here(self): # a very hard to please pet. But, I am kind!
        if self.hunger <= self.hunger_threshold and self.boredom <= self.boredom_threshold:
            print "Nope! I am happy and satiated. Get a life and entertain yourself!"
        elif self.hunger > self.hunger_threshold:
            print "Is that my dinner?? If not, WHY R U HERE? GO GET MY FOOD!"
            self.hunger -= 3
        else:
            print "That's the toy from last Christmas. Don't be so stingy. Buy me a new one."
            self.boredom -= 5
...
# Next, make pets respond to one additional command, of the form "Here <petname>". 
# When given that command, make the pet do something (i.e., print something and/or change its state). 
# Make it do something different depending on whether it is hungry or bored or not. Feel free to be creative here.
