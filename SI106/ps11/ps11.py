...
###
flickr_key = 'c61512377491174caea4e3cea7bd142c' # paste your flickr key here
...
search = flickrREST(extra_params = {'tags':tag, 'per_page':50})
# print( search.url )
search_result = json.loads( fix_flickr_resp( search.text ) )
# print( pretty(search_result) )

# [PROBLEM 3] Extract each of the photo ids from that response, making a list of all of them.
photo_list = search_result['photos']['photo']
id_list = [ photo['id'] for photo in photo_list]
# print( id_list )
...
try:
    for id in id_list:
        search = flickrREST(method = 'flickr.photos.getInfo', extra_params = { 'photo_id': id })
        search_results.append( search )

    for search in search_results:
        search_result = json.loads( fix_flickr_resp( search.text ) )
        for t in search_result['photo']['tags']['tag']:
            if t['_content'] in tag_freq:
                tag_freq[ t['_content'] ] += 1
            else:
                tag_freq[ t['_content'] ] = 1
except Exception, e:
...
sortedTagsByFrequencies = [ t[0] for t in  sorted( tag_freq.items(), key = lambda t : t[1], reverse = True ) ]
# print( sortedTagsByFrequencies )
...
print( sortedTagsByFrequencies[1 : 6] )
