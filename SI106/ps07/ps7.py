...
words = f.readlines() # get words
f.close() # close file
parsedWords = list( w.rstrip('\n') for w in words if 'z' in w ) # remove all \n in words and words with 'z' char in it
parsedWords = list( w for w in parsedWords if len(w) == 3 ) # filter out word with length of 3 only
short_z_words = sorted( parsedWords, reverse = True ) # sort the result by reverse alphabetic order
...
f.close() # close file
parsedWords = list( w.rstrip('\n') for w in words if 'z' in w ) # remove all \n in words and words with 'z' char in it
# setWords = list( ''.join(set(w)) for w in parsedWords )
scoreResult = []
...
zipped = zip( parsedWords, scoreResult )
best_z_words = sorted(zipped, key = lambda z : z[1], reverse = True)[:10]
best_z_words = list(x[0] for x in best_z_words)
