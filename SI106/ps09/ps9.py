...
# Create a derived class from BallDeflector
class Brick(BallDeflector):
    # Create a method called deflect_ball  
    def deflect_ball(self, ball, side_hit):
        super(Brick, self).deflect_ball(ball, side_hit)
        self.game.game_objects.remove(self)
        self.game.increment_hit_count()
        print("hit a bick and total hit is: " + str(self.game.hit_count))
        ...
        for i in range(6):
            self.bricks.append(
                Brick(
                    ...
                    game = self ))
            for j in range(450//40):
                self.bricks.append(
                Brick(
                    initial_x = 750 - (i*40),
                    initial_y = j * brick_height,
                    img_file = wall_imgs[2],
                    game = self))
...
class GameWindow(pyglet.window.Window):
    ...
    def __init__(self, ball_img, paddle_imgs, wall_imgs, 
        ...
        self.fps = 40 #Number of frames per seconds
